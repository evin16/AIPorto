...
# browseFiles
def browseFiles():
    browseFiles.filename = filedialog.askopenfilename(initialdir="/", title="Encripsi/DescripsiFile",)
    label_file_explorer.configure(text="File yang dibuka dalam: " + browseFiles.filename)
    pass_label.pack()
    password.pack()
    temp_label.pack()
    button_encrypt.pack()
...
def encrypt_file(p_word):
    ...
    status_label.configure(text="File telah dienkripsi")
    status_label.pack()
...
def decrypt_file(p_word):
    ...
    status_label.configure(text="Proses telah didekripsi")

# GUI
window = Tk()
window.title('Encripsi/DescripsiFile')
...
credit = Label(window,text = "Developed by admin", bg="black",height=2,  fg = "white", font =("",15))
...
button_explore = Button(window, text="Masukkan file", command=browseFiles, width=button_width, height=button_height, font =("",15))
password = Entry(window, textvariable=passwd,show="*")
button_encrypt = Button(window, text="Encrypt", command=submit_para_en, width=button_width, height=button_height, font =("",12))
button_decrypt = Button(window, text="Decrypt", command=submit_para_de, width=button_width, height=button_height, font =("",12))
